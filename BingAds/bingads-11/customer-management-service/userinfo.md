...
|<a name="username"></a>UserName|The logon user name of the user.<br/><br/>If [multi-user credentials](../guides/customer-accounts.md#multi-user) were provisioned through the user invitation work flow i.e., there was never an "old user name" for access to a customer, a system generated GUID will be returned in the *UserName* element and you cannot authenticate with this user in Bing Ads API version 11. In version 12 the multi-user email address will be returned and those credentials can be used to authenticate.|**string**|
